...
export const CREATE_DEPARTMENT = async (req, res) => {
  const { name } = req.body;
  const department = await Department.create({ name, companyId: req.user.userId });
  res.status(200).json({ message: "Department created successfully", department });
};

export const GET_DEPARTMENTS = async (req, res) => {
  const departments = await Department.find({ companyId: req.user.userId });
...
export const DEACTIVATE_DEPARTMENT = async (req, res) => {
  const { id } = req.params;
  const department = await Department.findByIdAndUpdate(id, { status: "inactive" });
  res.status(200).json({ message: "Department deactivated successfully", department });
};
